		...
		if (menuComponent != null) {
			if (menuComponent instanceof HMButton) {
				if (checkButton((HMButton) menuComponent, event)) {
					add(hybridMenu, HMButton.get()
											.withCaption(((HMButton) menuComponent)
											.getText())
											.withStyleName("clickable")
											.withThemeName("bcButton")
											.withClickListener(e -> ((HMButton) menuComponent).click()), menuContentList);
				...
				if (menuComponent instanceof HMSubMenu) {
					...
					if (foundActiveButton) {
						add(hybridMenu, HMButton.get()
												.withCaption(hmSubMenu.getButton().getText())
												.withThemeName("bcButton"), menuContentList);
						hmSubMenu.open();
					} else {
						hmSubMenu.close();
					}
